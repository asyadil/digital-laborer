...
class DatabaseSessionManager:
    ...
    def session_scope(
        ...
        """Provide a transactional scope with retry for lock-related errors."""
        attempt = 0
        while True:
            attempt += 1
            ...
            try:
                ...
                is_lock = "database is locked" in message or "deadlock" in message
                ...
                if logger:
                    logger.error(
                        "Operational error during DB session",
                        extra={"component": "db", "attempt": attempt, "error": str(exc)},
                    )
                raise
